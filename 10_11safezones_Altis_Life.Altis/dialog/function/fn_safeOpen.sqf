...
if((life_safeObj getVariable["safe",-1]) < 3) exitWith {hint localize "STR_Civ_VaultEmpty";};
if((life_safeObj getVariable["inUse",false])) exitWith {hint localize "STR_Civ_VaultInUse"};
if({side _x == west} count playableUnits < 3) exitWith {hint localize "STR_Civ_NotEnoughCops"};
